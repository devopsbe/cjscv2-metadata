...
// Load static trait data (bundled with the server)
let staticTraits = {};
try {
  staticTraits = require("../data/traits.json");
  console.log(`Loaded static traits for ${Object.keys(staticTraits).length} tokens`);
} catch (error) {
  console.warn("Warning: Could not load static traits file:", error.message);
}
...
// IPFS URLs (trim to handle any whitespace from env var parsing)
const STATIC_IMAGE_BASE = (process.env.STATIC_IMAGE_IPFS || "ipfs://QmUBoa8gPCN3NhWj9txpCF5FwToUq1NMN3YZ7MBwkjzQwQ/").trim();
const ANIMATION_OVERLAY = (process.env.ANIMATION_OVERLAY_IPFS || "ipfs://bafybeidngluwswiusroipprouoslpam2gtxcbryj54ljk7br3nsrrup2ke").trim();
const ANIMATED_IMAGE_BASE = process.env.ANIMATED_IMAGE_IPFS ? process.env.ANIMATED_IMAGE_IPFS.trim() : null;
...
async function buildMetadata(tokenId) {
  ...
  const staticImage = `${STATIC_IMAGE_BASE}${tokenId}.png`;
  ...
  // Build attributes - start with static traits from generated metadata
  const attributes = [];
  
  // Add static visual traits (Background, Body, Eyes, etc.)
  const tokenStaticTraits = staticTraits[tokenId.toString()] || [];
  attributes.push(...tokenStaticTraits);
  
  // Add dynamic on-chain attributes
  attributes.push(
    {
      trait_type: "Rarity",
      value: rarityName
